...
let incr_btn: Chemistry.Polygon;
let weight: HTMLInputElement;
let magnetic_fld: HTMLInputElement;
let specimen: HTMLInputElement;
let probe_btn_status: boolean;

let reading_count = 0;
let tab: HTMLTableElement;
let h_inp: HTMLInputElement;
let in_wt_inp: HTMLInputElement;
let fi_wt_inp: HTMLInputElement;
...
function activity2() {
   ...
   <div>
	...
	incr_btn = new Chemistry.Polygon(
		new Chemistry.Point(530, 125),
		20,
		3,
		canvas
	);
...
function specimen_selected(val: string) {
	console.log(val);
	specimen = <HTMLInputElement>document.getElementById('specimen');
	weight = <HTMLInputElement>document.getElementById('weight');
	magnetic_fld = <HTMLInputElement>document.getElementById('mag-field');
	in_wt_inp = <HTMLInputElement>(
		document.getElementById(`a2-in-wt-inp-${reading_count + 1}`)
	);
	...
	pp.showdescription(
		`<p class='discription_text'>Note initital weight to observation table</p>
...
function verify_reading() {
	h_inp = <HTMLInputElement>(
		document.getElementById(`a2-H-inp-${reading_count}`)
	);
	in_wt_inp = <HTMLInputElement>(
		document.getElementById(`a2-in-wt-inp-${reading_count}`)
	);
	fi_wt_inp = <HTMLInputElement>(
		document.getElementById(`a2-fi-wt-inp-${reading_count}`)
	);
	...
	if (reading_count < 5) {
		pp.showdescription(
			`<p class='discription_text'>For reading ${
				reading_count + 1
			}, note given initial weight to observation table.</p>
         <p class='discription_text'>Then put probe on wooden stand</p>
         `,
			3
		);
	...
	let temp_inp: HTMLInputElement = <HTMLInputElement>(
		document.getElementById(`a2-in-wt-inp-${reading_count + 1}`)
	);
	if (reading_count < 5) {
...
function a2_mouse_click_increase_btn(e: MouseEvent) {
	...
	if (y >= 115 && y <= 144) {
		if (x >= 510 && x <= 547) {
			...
			pp.showdescription(
				`<p class='discription_text'>Note magnetic field to observation table</p>
...
function animateTimer() {
	if (selected_specimen === 1) {
		...
		if (currentTime <= final_wt[reading_count - 1]) return;
	} else {
		currentTime = parseFloat((currentTime + 0.001).toFixed(3));
		weight.value = currentTime.toString() + ' g';
...
function internal_calculations(value: number) {
	if (value === 1) {
		final_wt = final_wt_Ca.map((data, index) =>
			index === 0
				? data
		...
		final_wt = final_wt_K.map((data, index) =>
			index === 0
				? data
...
function load_full_obervation_table() {
	pp.showdescription(
		`<p class='discription_text'>Simulation complete. Click next for calculations.</p>
	   `,
		3
	);
	pp.addtorightpannel(
	...
	let table_div = <HTMLDivElement>document.getElementById('a2-table-1');
...
function a2_mouse_click_probe(e: MouseEvent) {
	let x = Math.round((e.clientX - rect.x) / lscale);
	let y = Math.round((canvas.height - (e.clientY - rect.y)) / lscale);
	...
	if (probe_btn_status) {
		if (y >= 74 && y <= 281) {
			if (x >= 965 && x <= 994) {
				...
				h_inp = <HTMLInputElement>(
					document.getElementById(`a2-H-inp-${reading_count + 1}`)
				);
            ...
            `,
					...
					canvas.addEventListener(
						'click',
						a2_mouse_click_increase_btn
					);

					pp.showdescription(
						`<p class='discription_text'>Increase electromagnet power supply</p>
            `,
						3
					);
					show_panel(3);
		...
		if (y >= 206 && y <= 358) {
			if (x >= 761 && x <= 778) {
				...
				if (reading_count > 1) animateTimer();
               ...
               `,
				...
				pp.addtorightpannel(
					`<button id="panel1_btn" class="btn btn-primary" onclick="verify_reading()" style="position: absolute; bottom: 12vh; width: 91%;">Next</button>`,
					3
				);
				show_panel(3);
