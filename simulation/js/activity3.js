function activity3() {
    ...
    pp.showdescription(`
      ...
      `, 3);
    pp.addtorightpannel(`<button  class="btn btn-primary" onclick="a3_verify_table()" style="position: absolute; bottom: 12vh; width: 91%;">Next</button>`, 3);
    show_panel(3);
    a3_load_table1();
...
function a3_verify_table() {
    ...
    let susc_inp;
    for (let i = 1; i < 6; i++) {
        h_sq_inp = (document.getElementById(`a3-H-sq-inp-${i}`));
        dm_inp = document.getElementById(`a3-dm-inp-${i}`);
        susc_inp = document.getElementById(`a3-sus-inp-${i}`);
...
function a3_load_data_table() {
    ...
    pp.showdescription(`<p class='discription_text'>Calculations correct. Calculate average susceptibility.</p>`, 3);
    show_panel(3);
    let tbody = ``;
    for (let i = 1; i < 11; i++) {
        tbody += `<tr>
...
function verify_average_susceptibility() {
    let avg_susc_inp = document.getElementById('avg-susc');
    let btn = document.getElementById('avg-susc-btn');
    ...
    if (!verify_values(parseFloat(avg_susc_inp.value), susceptibility)) {
        ...
        pp.showdescription(`<p class='discription_text'>Calculations correct. Click on next.</p>`, 3);
        pp.addtorightpannel(`<button id="panel1_btn" class="btn btn-primary" onclick="activity4()" style="position: absolute; bottom: 12vh; width: 91%;">Next</button>`, 3);
        show_panel(3);
    }
    btn.remove();
