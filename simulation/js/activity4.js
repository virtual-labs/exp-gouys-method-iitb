function activity4() {
    ...
    let text = `
      <div style="margin-top:10px;">
         <h4>Standard value of Susceptibility = ${selected_specimen === 1
        ? '-38.2 &times; 10<sup>-6</sup>'
        : '20 &times; 10<sup>-6</sup>'} cm<sup>3</sup>/mol </h4>
      </div>
      <br>
      <br>
...
function verify_cal_susceptibility() {
    let val = (document.getElementById('cal-susc'));
    let div = (document.getElementById('cal-susc-div'));
    ...
    if (!verify_values(parseFloat(val.value), susceptibility)) {
        ...
        div.innerHTML = '';
        div.innerHTML = `
      <h4>Calculated value of Susceptibility = ${susceptibility * Math.pow(10, 6)} &times; 10<sup>-6</sup> cm<sup>3</sup>/mol </h4>
      `;
    }
...
function experiment_complete() {
    let btn = document.getElementById('cal-susc-btn');
