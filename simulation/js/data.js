...
let susc_compare; // camparision btwn cal susc & standrd susc
...
let K = [[0, 0, 5, 5, 0, 0]];
// ========== calculations ========== //
function calculate_susceptibility(data) {
    let susc_sum = 0;
    for (let i = 1; i < data.length; i++) {
        let sus = parseFloat(((2 * data[i][4] * g) / (A * data[i][1])).toFixed(6));
        data[i].push(sus);
        susc_sum += sus;
    }
    susceptibility = parseFloat((susc_sum / (data.length - 1)).toFixed(6));
}
function cal_data(arr, final_wt) {
    for (let i = 1; i < H.length; i++) {
        let ar = [];
        ar.push(H[i]);
        ar.push(Math.pow(H[i], 2));
        ar.push(initial_wt);
        ar.push(final_wt[i]);
...
function cal_percentage_error(standard_susc) {
    let error = Math.round(Math.abs((susceptibility - standard_susc) / standard_susc) * 100);
